    ...
    const ticketsHtml = tickets.map(ticket => {
      ...
      return `
       <div style="display:flex; gap:16px; flex-wrap:wrap; margin-top:16px;">
        <img src="${ticket.imageUrl || ticket.url}" alt="${ticket.title || ticket.titulo}" 
             style="width:50%; max-width:300px; height:auto; border-radius:8px; object-fit:contain;" />
        <ul style="list-style:none; padding:0; margin:0; width:50%; font-size:14px; line-height:1.6;">
    ...
    const mailHtml = `
      <div style="font-family: Arial, Helvetica, sans-serif; max-width:650px; margin:40px auto; padding:24px; color:#333; background-color:#f9f9f9; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
        <div style="display:flex; width:100%; border-bottom:1px solid #ccc; padding-bottom:8px; margin-bottom:8px;">
          <div style="width:33%; text-align:left; font-size:18px; font-weight:bold;">CIRCULO ESCENA ® SAPI de CV</div>
          <div style="width:33%; text-align:center; font-size:18px;">Orden de Compra</div>
          <div style="width:33%; text-align:right; font-size:14px; font-weight:bold;">#${orden}</div>
        </div>
        <div style="display:flex; width:100%; margin-bottom:16px;">
          <div style="width:50%; text-align:left; color:green; font-weight:bold; font-size:15px;">Compra Aprobada</div>
          <div style="width:50%; text-align:right; font-size:14px;">${fechaActual}</div>
        </div>
        ...
        <div style="display:flex; width:100%; justify-content:space-between; align-items:flex-start; gap:16px; margin-bottom:16px;">
          ...
          <div style="width:50%; text-align:right;">
            <img src="${process.env.URL_LOGO}" alt="Logo" style="width:120px; max-width:120px; opacity:0.5; border:1px solid #ccc; padding:4px; border-radius:4px;">
          </div>
        </div>
